...
for key, value in keithleys.items():
    ...
    #X-axis
    g1.GetXaxis().SetTitle("#font[22]{time [hh:mm]}")
    g1.GetXaxis().CenterTitle()
    g1.GetXaxis().SetTimeFormat("%H:%M")
    g1.GetXaxis().SetTimeOffset(1486249200)
    g1.GetXaxis().SetTimeDisplay(1)
    g1.GetXaxis().SetTitleSize(0.05)
    g1.GetXaxis().SetLabelSize(0.05)
    g1.GetXaxis().SetTitleOffset(0.90)
    #Y-axis
    g1.GetYaxis().SetTitleOffset(0.69)
    g1.GetYaxis().SetTitle("#font[22]{current [nA]}")
    ...
    h1 = c.DrawFrame(min(xx[key]), min(current[key])-dy, max(xx[key]), max(current[key])+dy)
    ...
    h2 = p2.DrawFrame(min(xx[key]), -1100, max(xx[key]), 1100)
    ...
    a1 = ROOT.TGaxis(max(xx[key]), -1100, max(xx[key]), 1100,-1100,1100,510,"+L")
